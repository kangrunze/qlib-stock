"""
CatBoost model implementation for the Stock-AI quantitative trading project.

Wraps catboost.CatBoostRegressor with configuration loading, early stopping,
...
class CatBoostModel(BaseModel):
    """CatBoost gradient boosting model wrapper.

    Wraps catboost.CatBoostRegressor with support for:
    ...
    Attributes:
        model: The underlying catboost.CatBoostRegressor instance.
    ...
    def _load_config(self, config_path: Optional[str] = None) -> Dict[str, Any]:
        """Load CatBoost parameters from YAML configuration file.
        ...
        try:
            ...
            params = config.get("catboost", {})
            logger.info("Loaded CatBoost config from %s: %s", path, params)
            return dict(params)
    ...
    def fit(
        ...
        """Fit CatBoost model on training data with optional early stopping.
        ...
        logger.info(
            "Fitting CatBoost model: X shape=%s, y shape=%s", X.shape, y.shape
        )
        ...
        # CatBoost handles NaN internally, but we've already cleaned them
        # for consistency
        ...
        if X_valid is not None and y_valid is not None:
            ...
            logger.info("Early stopping enabled via validation set")
        else:
    ...
    def get_feature_importance(self) -> pd.DataFrame:
        ...
        CatBoost provides multiple importance types. Default uses
        'FeatureImportance' which is the default importance calculation.
        ...
        try:
            ...
            logger.warning("Model does not have get_feature_importance method")
