"""
XGBoost model implementation for the Stock-AI quantitative trading project.

Wraps xgboost.XGBRegressor with configuration loading, early stopping,
...
class XGBoostModel(BaseModel):
    """XGBoost gradient boosting model wrapper.

    Wraps xgboost.XGBRegressor with support for:
    ...
    def _load_config(self, config_path: Optional[str] = None) -> Dict[str, Any]:
        """Load XGBoost parameters from YAML configuration file.
        ...
        try:
            ...
            logger.info("Loaded XGBoost config from %s: %s", path, params)
            return dict(params)
    ...
    def fit(
        ...
        """Fit XGBoost model on training data with optional early stopping.
        ...
        logger.info(
            "Fitting XGBoost model: X shape=%s, y shape=%s", X.shape, y.shape
        )
        ...
        if X_valid is not None and y_valid is not None:
            ...
            logger.info(
                "Early stopping enabled via validation set"
            )
        else:
    ...
    def get_feature_importance(self) -> pd.DataFrame:
        ...
        Supports both 'weight' and 'gain' importance types.

        Returns:
            DataFrame with columns 'feature' and 'importance', sorted
            by importance (gain) in descending order.
