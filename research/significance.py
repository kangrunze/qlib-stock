...
def ic_significance_report(
    ...
    Returns:
        格式化的报告字符串
    """
    result = newey_west_test(ic_series, max_lags=max_lags, alpha=alpha)
    ...
    lines = [
        ...
        f"  IC 均值:         {result['ic_mean']:.6f}" if result['ic_mean'] is not None else "  IC 均值:         N/A",
        f"  IC 标准差:       {result['ic_std']:.6f}" if result['ic_std'] is not None else "  IC 标准差:       N/A",
        f"  朴素标准误:      {result['ic_std'] / np.sqrt(result['n_samples']):.6f}" if result['ic_std'] is not None and result['n_samples'] > 0 else "  朴素标准误:      N/A",
        f"  NW 滞后阶数:     {result['max_lags']}",
        f"  NW 调整标准误:   {result['nw_se']:.6f}" if result['nw_se'] is not None else "  NW 调整标准误:   N/A",
        f"  t 统计量(朴素):  {result['t_stat']:.4f}" if result['t_stat'] is not None else "  t 统计量(朴素):  N/A",
        f"  t 统计量(NW):    {result['t_nw']:.4f}" if result['t_nw'] is not None else "  t 统计量(NW):    N/A",
        f"  p 值(朴素):      {result['p_value_naive']:.6f}" if result['p_value_naive'] is not None else "  p 值(朴素):      N/A",
        f"  p 值(NW):        {result['p_value_nw']:.6f}" if result['p_value_nw'] is not None else "  p 值(NW):        N/A",
        f"  显著性阈值:      |t_NW| > 2.0",
        "-" * 60,
    ]
